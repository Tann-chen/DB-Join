...
public class QuickSort {
   ...
   */
  public static void quicksort(byte[][] data, int start, int end) {
...
//      TestSort.printArray(data);
  ...
  public static void insertionsort(byte[][] data, int start, int end) {
    for (int next = start + 1; next <= end; next++) {
      if (compare(data[next-1],data[next]) > 0) {
        byte[] val = data[next];
        int gap = next-1;
        data[next] = data[gap];
        while (gap > start && compare(data[gap-1],val) > 0) {
          data[gap] = data[gap-1];
          gap--;
        }
        data[gap] = val;
